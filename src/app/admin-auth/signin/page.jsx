...
import { loginAdminSchema } from "@/validation/loginSchema";
...
const Signin = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const response = await api.post("/loginSuperAdmin", {
                email: data.email,
                password: data.password
            });

            if (response?.data?.success) {
                showSuccessToast(response?.data?.message || "Account LoggedIn successfully");
                ...
                if (response?.data?.token) {
                    ...
                    router.push("/admin"); // redirect to OTP page
                }
                ...
                <form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
                    ...
                    <div className="text-start mt-4">
                        ...
                        <div className="register_link">
                            <h5>
                                Already have an account? <Link href="/admin-auth/signup">Sign Up</Link>
                            </h5>
                        </div>
                    </div>
                </form>
